...
class BaseNet(nn.Module):
    ...
    def forward(self, sentence, batch_size):
        ...
        output, (hn, cn) = self.lstmm(self.embedding(sentence))  #
        #self.hidd = (hn, cn)
    ...
    def fit(self, train_iterator, test_iterator):
    ...
    def predict(self, test_iterator):
        ...
        total = 1488
        print('Accuracy of the network on the 1488 test images: %d %%' % (
            100 * correct / total))
        #print("Corr", correct)
...
def main():
    ...
    train_iterator, test_iterator, len1 = load_dataset("questions", batch_size)
    ...
    plot_confusion_matrix(pred_labels, test_labels, "LSTM_Question")
    plt.show()
    plt.savefig('ConfusionMatrix_LSTM_Question.png', )
    torch.save(model, 'LSTM_Question.pt')
